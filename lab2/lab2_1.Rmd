...
The mean of new_vector could not be calculated because it contains an NA, which is not a numerical value that can be calculated with, so mean(new_vector) just outputs an NA. This means that NA is not zero, which would have been usable for calculations.
...
This is useful since it removes all the NA values so that it doesn't interfere with calculations that we want to make with the rest of our data, instead allowing for proper analyses of all of the available data.
