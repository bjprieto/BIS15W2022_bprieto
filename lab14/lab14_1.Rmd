...
penguins %>% 
  ...
  coord_flip()
```
Well that didn't really reveal the reason for the bimodal shape on Biscoe. 
...
homerange %>% 
  ggplot(aes(x = taxon, y = log10.mass))+
  geom_violin(aes(fill = taxon), alpha = 0.8)+
  labs(x = "Taxon",
       y = "Log 10 Mass",
       title = "Log 10 Mass by Taxon",
       fill = "Taxon")+
  geom_boxplot(data = homerange, aes(x = taxon, y = log10.mass, fill = taxon), alpha = 0.2)+
...
## Practice
  ...
  Drug D lowers heart rate, drug C raises it.
