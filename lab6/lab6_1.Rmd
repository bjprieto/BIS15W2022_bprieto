...
NA means not applicable/not available. NA's are treated as a lack of data that can't be used in computations.
```{r}
...
```
The problem with Cetacea is that none of the Cetacea entries for brainwt are numerical values, so the mean brainwt for Cetacea is not a number (NaN). In contrast, Carnivora has a combination of numerical values and NAs, so when NAs are removed there is still data to compute a mean.
